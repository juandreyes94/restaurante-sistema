...
async function doLogin() {
  ...
  try {
    ...
    if (data.role !== _selRole) throw new Error(`Ese código no es de ${NOMBRE_ROL[_selRole] || _selRole}`);
    localStorage.setItem(AUTH_KEY, JSON.stringify({ role: data.role, token: data.token }));
    routeTo(data.role);
  } catch (e) {
    input.classList.add('err');
    if (err) err.textContent = e.message || 'Código incorrecto';
